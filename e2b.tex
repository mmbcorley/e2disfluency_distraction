...
%%%MC movements beyond edge:  any idea how many samples were rejected on this basis?
...
%%MC cite t to 2dp please, for consistency
...
When the car-horn was present prior to a disfluency, participants' tendency were more likely to move towards the referent over time ($\beta = 0.37$; $\SE = 0.05$; $t=6.73$). \fbox{see comment}
%%MC please include t for fluent utterances, as at end of eye movements para
...
%%%MC three reasons: (1) different experimental details; (2) we're talking about meaning, they're talking about prediction; (3) our task, by its nature, requires ppts to REASON about the speaker.  So one conclusions is that ppts CAN take context into account (when it matters), but may not always do so, perhaps because it doesn't matter or because cues are more ambiguous.

%%MC we also need to account for the fact that the basic disf-dishonesty bias wins out!!  I would say that that is due to the context of the experiment (drawing on the "need to reason" in the Arnold paragraph) and that what we're seeing is a tradeoff between fast processes and conscious reasoning.  I'd refer back to the 19 participants who made the link between disfluency and deception in post-test.

%%MC final paragraph should emphasise: (1) that, at least very early, ppts are sensitive to disfluencies and the context they occur in, and that this can be attributed to some form of speaker modelling (choose words carefully based on the "2nd explanation" para above?); (2) mouse tracking usefully gives information that is comparable to eye tracking; and (3) that in general, this shows that understanding a speaker's pragmatic intentions is a contextually-rich, and very fast, process.
